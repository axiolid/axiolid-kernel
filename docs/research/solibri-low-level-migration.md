...
| `profile/triangulate` | `axiolid-reference` / `axiolid-tessellate` | partial | Simple loops use the certified Axiolid scalar adapter; holes remain blocked pending a maintained constrained backend. |
...
| `solid/*` | `axiolid-mesh-boolean-boolmesh` / `axiolid-compile` | inventory | Do not import native CSG bindings or Solibri representation policy. |
