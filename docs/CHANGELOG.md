...
### Removed
- Removed the `axiolid-sweep` crate and the facade's `sweeps` feature and `axiolid::sweep` module. The crate held a single `Sweeper` trait with no implementors, no tests, and no references anywhere in the workspace, while sweep construction actually lives in `axiolid-compile`. Capability seams now live in `axiolid-kernel` alongside `MeshBoolean`; see [ADR 0021](./adr/0021-capability-seams-live-in-the-kernel.md).
